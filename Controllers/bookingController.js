...
exports.getCheckOutSession = catchAsync(async (request, response, next) => {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    }&user=${request.user.id}&price=${tour.price}`, //not secure at all lekin sirf dev phase tak karsatay, prod may iski jaga stripe web hooks use karenay to create booking in db
